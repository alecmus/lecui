...
HRESULT liblec::lecui::widgets_implementation::label::create_resources(
	...
	try {
		...
		do {
			...
			// capture text range
			properties.text_range = { static_cast<UINT32>(start_pos), static_cast<UINT32>(value.length()) };
	...
	if (SUCCEEDED(hr)) {
		p_text_format_->SetTextAlignment(specs_.center_h ? DWRITE_TEXT_ALIGNMENT_CENTER : DWRITE_TEXT_ALIGNMENT_LEADING);
		p_text_format_->SetParagraphAlignment(specs_.center_v ? DWRITE_PARAGRAPH_ALIGNMENT_CENTER : DWRITE_PARAGRAPH_ALIGNMENT_NEAR);
...
liblec::lecui::widgets_implementation::label::render(ID2D1HwndRenderTarget* p_render_target,
	...
	if (SUCCEEDED(hr)) {
		...
		for (auto& properties : formatting_) {
			...
			hr = p_render_target->CreateSolidColorBrush(properties.color, &p_brush);
			if (SUCCEEDED(hr))
				p_text_layout_->SetDrawingEffect(is_enabled_ ? p_brush : p_brush_disabled_, properties.text_range);
