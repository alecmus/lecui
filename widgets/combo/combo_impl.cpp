/*
** combo_impl.cpp - combo_impl implementation
**
...
namespace liblec {
	namespace lecui {
		widgets_impl::combo::combo(const std::string& page_alias,
			...
			color_scrollbar_({ 0, 120, 170, 150 }),
			color_scrollbar_hot_({ 0, 120, 170, 200 }),
			color_scrollbar_hot_pressed_({ 0, 120, 170, 255 }),
			color_scrollbar_background_({ 0, 120, 170, 50 }),
		...
		widgets_impl::combo::~combo() { discard_resources(); }

		widgets_impl::widget_type
			widgets_impl::combo::type() {
			return lecui::widgets_impl::widget_type::combo;
		}

		HRESULT widgets_impl::combo::create_resources(
		...
		void widgets_impl::combo::discard_resources() {
		...
		D2D1_RECT_F&
			widgets_impl::combo::render(ID2D1HwndRenderTarget* p_render_target,
		...
		void widgets_impl::combo::on_click() {
		...
		bool widgets_impl::combo::on_menu(ID2D1HwndRenderTarget* p_render_target,
		...
		void widgets_impl::combo::reset_menu() {
			// reset parameters
			draw_menu_ = false;
			move_by_ = 0;
			displacement_ = 0.f;
		...
		bool widgets_impl::combo::hit(const bool& hit) {
